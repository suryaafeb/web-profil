...
if(isset($_GET['data'])){
  ...
  $_SESSION['id_riwayat_pekerjaan'] = $id_riwayat_pekerjaan;

  // get data riwayat pekerjaan
  $sql_p = "SELECT tahun, posisi, id_master_perusahaan
  ...
  while($data_p = mysqli_fetch_row($query_p)){
    $tahun = $data_p[0];
    $posisi = $data_p[1];
    $id_master_perusahaan = $data_p[2];
  }

  // kalau misal ada relasi tambahan kayak tag atau kategori khusus pekerjaan bisa tambahkan query lain di sini.
}
?>
...
<?php }?>
      ...
      <form class="form-horizontal" action="konfirmasieditriwayatpekerjaan.php" method="post">
  <div class="card-body">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-7">
        <input type="text" class="form-control" name="posisi" id="posisi" value="<?php echo $posisi;?>">
      </div>
    </div>
        ...
        <select class="form-control" id="perusahaan" name="perusahaan">
          ...
          ?>
            <option value="<?php echo $id_master_perusahaan;?>"
              <?php if($id_master_perusahaan==$id_master_perusahaan_selected){?> selected <?php }?>>
